...
public class OldFutronicData {
    ...
    public static void methodOldFP() {
        ...
        try {
            ...
            allAgentNo.matching(null,status); // Receive all agent and customer no (agentNo against each customer)
   ...
   //Main() method for testing purpose  
    ...
    public static void oldAllAgent(String pStatus){
        StandardMatching allAgentNo = new StandardMatching();
        HashMap<Integer, List<Integer>> mapAgentCustNo = allAgentNo.matching(pStatus); // Receive all agent no of specific customer from 'StandardMatching' class which inherit here.
        try{
            System.out.println(mapAgentCustNo);
        }
        catch(Exception ex)
